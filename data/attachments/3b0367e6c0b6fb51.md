...
- Name: desktop/home.spec.ts >> Home – Style for Real Life tiles (HOME-301–308) >> HOME-302 body copy matches spec
- Location: tests/desktop/home.spec.ts:328:7
...
Locator: locator('section').filter({ hasText: 'Style for Real Life' }).first().getByText(/At Market Street, we believe fashion should be effortless/)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ hasText: 'Style for Real Life' }).first().getByText(/At Market Street, we believe fashion should be effortless/)
