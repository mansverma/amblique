...
- Name: desktop/plp.spec.ts >> Shared Product Card on PLP (CARD-101–116) >> CARD-102 brand name is present above product name
- Location: tests/desktop/plp.spec.ts:207:7
...
Locator: locator('[class*="product-card"]').first().locator('p.text-muted-foreground, p[class*="muted"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[class*="product-card"]').first().locator('p.text-muted-foreground, p[class*="muted"]').first()
