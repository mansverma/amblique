...
- Name: desktop/home.spec.ts >> Home – Featured Products carousel (HOME-201–206) >> HOME-203 first product in carousel is "Leather Crossbody Bag" (order per HOME-204)
- Location: tests/desktop/home.spec.ts:285:7
...
Locator: locator('.section-container, section').filter({ hasText: 'Featured Products' }).first().getByText('Leather Crossbody Bag').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.section-container, section').filter({ hasText: 'Featured Products' }).first().getByText('Leather Crossbody Bag').first()
