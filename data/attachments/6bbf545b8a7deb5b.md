...
- Name: mobile/plp.spec.ts >> Mobile PLP sort (M-PLP-107–109) >> M-PLP-107 sort control is a native select labelled "Sort by:"
- Location: tests/mobile/plp.spec.ts:64:7
...
Locator: locator('select').filter({ has: locator('option') }).first()
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('select').filter({ has: locator('option') }).first()
