...
- Name: desktop/plp.spec.ts >> Shared Product Card on PLP (CARD-101–116) >> CARD-105 SKU is present in format "SKU: {id}"
- Location: tests/desktop/plp.spec.ts:221:7
...
Locator: locator('[class*="product-card"], [data-product-id]').first().getByText(/SKU:/i)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[class*="product-card"], [data-product-id]').first().getByText(/SKU:/i)
