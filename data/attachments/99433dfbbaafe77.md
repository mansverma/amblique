# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: desktop/plp.spec.ts >> PLP pagination and grid (PLP-114–118) >> PLP-114 Women page shows 12 product cards
- Location: tests/desktop/plp.spec.ts:159:7

# Error details

```
Error: expect(locator).toHaveCount(expected) failed

Locator:  locator('[data-product-id], .product-tile, article[itemtype*="Product"]').or(locator('[class*="product-card"], [class*="ProductCard"]'))
Expected: 12
Received: 24
Timeout:  5000ms

Call log:
  - Expect "toHaveCount" with timeout 5000ms
  - waiting for locator('[data-product-id], .product-tile, article[itemtype*="Product"]').or(locator('[class*="product-card"], [class*="ProductCard"]'))
    5 × locator resolved to 4 elements
      - unexpected value "4"
    9 × locator resolved to 24 elements
      - unexpected value "24"

```

# Page snapshot

```yaml
- generic [active] [ref=e1]:
  - link "Skip to main content" [ref=e2] [cursor=pointer]:
    - /url: "#main-content"
  - banner [ref=e3]:
    - generic [ref=e5]:
      - link [ref=e6] [cursor=pointer]:
        - /url: /MarketStreet/en-US/
        - img "Home" [ref=e7]
      - navigation "Main" [ref=e10]:
        - list [ref=e12]:
          - listitem [ref=e13]:
            - button "Women" [ref=e14] [cursor=pointer]
          - listitem [ref=e15]:
            - button "Men" [ref=e16] [cursor=pointer]
          - listitem [ref=e17]:
            - button "Kids" [ref=e18] [cursor=pointer]
          - listitem [ref=e19]:
            - link "New Arrivals" [ref=e20] [cursor=pointer]:
              - /url: /MarketStreet/en-US/category/new-arrivals
      - generic [ref=e24]:
        - generic [ref=e25]: Search
        - combobox "Search" [ref=e26]
      - generic [ref=e27]:
        - button "Find a Store" [ref=e28] [cursor=pointer]
        - link "Sign In" [ref=e29] [cursor=pointer]:
          - /url: /MarketStreet/en-US/login
        - link "Wishlist" [ref=e30] [cursor=pointer]:
          - /url: /MarketStreet/en-US/wishlist
        - 'button "My cart, number of items: 0" [ref=e31] [cursor=pointer]'
  - main [ref=e32]:
    - generic [ref=e33]:
      - generic [ref=e40]:
        - generic [ref=e41]: Women
        - paragraph [ref=e42]: Women
        - generic [ref=e43]: 36 products available
      - generic [ref=e44]:
        - navigation "Breadcrumb" [ref=e46]:
          - list [ref=e47]:
            - listitem [ref=e48]:
              - link "Home" [ref=e49] [cursor=pointer]:
                - /url: /MarketStreet/en-US/
            - listitem [ref=e50]:
              - link "Women" [ref=e53] [cursor=pointer]:
                - /url: /MarketStreet/en-US/category/women
        - generic [ref=e54]:
          - heading "Women (36)" [level=1] [ref=e55]
          - generic [ref=e57]:
            - generic [ref=e58]: "Sort by:"
            - combobox "Sort by:" [ref=e60] [cursor=pointer]:
              - option "Best Matches" [selected]
              - option "Price Low To High"
              - option "Price High to Low"
              - option "Product Name A - Z"
              - option "Product Name Z - A"
              - option "Brand"
              - option "Most Popular"
              - option "Top Sellers"
        - generic [ref=e62]:
          - generic [ref=e63]:
            - button "Filters" [ref=e64] [cursor=pointer]
            - group "Quick category filters" [ref=e65]:
              - button "Accessories" [ref=e66] [cursor=pointer]
              - button "Bags" [ref=e67] [cursor=pointer]
              - button "Bottoms" [ref=e68] [cursor=pointer]
              - button "Dresses" [ref=e69] [cursor=pointer]
              - button "Knitwear" [ref=e70] [cursor=pointer]
              - button "New In" [ref=e71] [cursor=pointer]
              - button "Outerwear" [ref=e72] [cursor=pointer]
              - button "Shoes" [ref=e73] [cursor=pointer]
              - button "Tops" [ref=e74] [cursor=pointer]
          - generic [ref=e75]:
            - generic [ref=e76] [cursor=pointer]:
              - generic [ref=e78]:
                - link [ref=e80]:
                  - /url: /MarketStreet/en-US/product/standard-prd-womens-leather-crossbody-bag
                - link [ref=e84]:
                  - /url: /MarketStreet/en-US/product/standard-prd-womens-leather-crossbody-bag
                - button "Add to Wishlist" [ref=e87]
                - button "Quick Add Leather Crossbody Bag" [ref=e91]: Quick Add
              - generic [ref=e92]:
                - paragraph [ref=e93]: Performer
                - paragraph [ref=e94]: Women
                - heading [level=3] [ref=e95]:
                  - link "Leather Crossbody Bag" [ref=e96]:
                    - /url: /MarketStreet/en-US/product/standard-prd-womens-leather-crossbody-bag
                - paragraph [ref=e97]: "SKU: standard-prd-womens-leather-crossbody-bag"
                - generic [ref=e100]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e101]
                  - button "(218)" [ref=e112]
                - generic [ref=e114]:
                  - text: $109.99
                  - generic [ref=e115]: "Leather Crossbody Bag Current price: $109.99"
                  - text: $139.90
                  - generic [ref=e116]: "Leather Crossbody Bag List price: $139.90"
            - generic [ref=e117] [cursor=pointer]:
              - generic [ref=e119]:
                - link [ref=e121]:
                  - /url: /MarketStreet/en-US/product/womens-knit-midi-skirt?color=beige
                - link [ref=e125]:
                  - /url: /MarketStreet/en-US/product/womens-knit-midi-skirt?pid=womens-knit-midi-skirt-beige-xs
                - button "Add to Wishlist" [ref=e128]
                - button "Quick Add Knit Midi Skirt" [ref=e132]: Quick Add
              - generic [ref=e133]:
                - group "Available colors" [ref=e135]:
                  - link "View Knit Midi Skirt in Beige" [ref=e136]:
                    - /url: /MarketStreet/en-US/product/womens-knit-midi-skirt?color=beige
                  - link "View Knit Midi Skirt in Black" [ref=e137]:
                    - /url: /MarketStreet/en-US/product/womens-knit-midi-skirt?color=black
                  - link "View Knit Midi Skirt in Navy" [ref=e138]:
                    - /url: /MarketStreet/en-US/product/womens-knit-midi-skirt?color=navy
                - paragraph [ref=e139]: Performer
                - paragraph [ref=e140]: Women
                - heading [level=3] [ref=e141]:
                  - link "Knit Midi Skirt" [ref=e142]:
                    - /url: /MarketStreet/en-US/product/womens-knit-midi-skirt?pid=womens-knit-midi-skirt-beige-xs
                - paragraph [ref=e143]: "SKU: womens-knit-midi-skirt"
                - generic [ref=e146]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e147]
                  - button "(218)" [ref=e158]
                - generic [ref=e160]:
                  - text: $59.99
                  - generic [ref=e161]: "Knit Midi Skirt Current price: $59.99"
                  - text: $69.90
                  - generic [ref=e162]: Knit Midi Skirt List price from $69.90
            - generic [ref=e163] [cursor=pointer]:
              - generic [ref=e165]:
                - link [ref=e167]:
                  - /url: /MarketStreet/en-US/product/womens-denim-midi-skirt
                - link [ref=e171]:
                  - /url: /MarketStreet/en-US/product/womens-denim-midi-skirt?pid=womens-denim-midi-skirt-xs
                - button "Add to Wishlist" [ref=e174]
                - button "Quick Add Denim Midi Skirt" [ref=e178]: Quick Add
              - generic [ref=e179]:
                - paragraph [ref=e180]: Performer
                - paragraph [ref=e181]: Women
                - heading [level=3] [ref=e182]:
                  - link "Denim Midi Skirt" [ref=e183]:
                    - /url: /MarketStreet/en-US/product/womens-denim-midi-skirt?pid=womens-denim-midi-skirt-xs
                - paragraph [ref=e184]: "SKU: womens-denim-midi-skirt"
                - generic [ref=e187]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e188]
                  - button "(218)" [ref=e199]
                - generic [ref=e200]:
                  - generic [ref=e201]:
                    - text: $44.99
                    - generic [ref=e202]: "Denim Midi Skirt Current price: $44.99"
                    - text: $69.90
                    - generic [ref=e203]: Denim Midi Skirt List price from $69.90
                  - generic [ref=e204]: New In - 25% Off!
            - generic [ref=e206] [cursor=pointer]:
              - generic [ref=e208]:
                - link [ref=e210]:
                  - /url: /MarketStreet/en-US/product/womens-structured-blazer
                - link [ref=e214]:
                  - /url: /MarketStreet/en-US/product/womens-structured-blazer?pid=womens-structured-blazer-xs
                - generic [ref=e215]:
                  - generic [ref=e216]: Best Seller
                  - generic [ref=e217]: New
                - button "Add to Wishlist" [ref=e220]
                - button "Quick Add Structured Blazer" [ref=e224]: Quick Add
              - generic [ref=e225]:
                - paragraph [ref=e226]: Performer
                - paragraph [ref=e227]: Women
                - heading [level=3] [ref=e228]:
                  - link "Structured Blazer" [ref=e229]:
                    - /url: /MarketStreet/en-US/product/womens-structured-blazer?pid=womens-structured-blazer-xs
                - paragraph [ref=e230]: "SKU: womens-structured-blazer"
                - generic [ref=e233]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e234]
                  - button "(218)" [ref=e245]
                - generic [ref=e246]:
                  - generic [ref=e247]:
                    - text: $74.99
                    - generic [ref=e248]: "Structured Blazer Current price: $74.99"
                    - text: $129.90
                    - generic [ref=e249]: Structured Blazer List price from $129.90
                  - generic [ref=e250]: New In - 25% Off!
            - generic [ref=e252] [cursor=pointer]:
              - generic [ref=e254]:
                - link [ref=e256]:
                  - /url: /MarketStreet/en-US/product/womens-wool-blend-coat?color=beige
                - link [ref=e260]:
                  - /url: /MarketStreet/en-US/product/womens-wool-blend-coat?pid=womens-wool-blend-coat-beige-xs
                - button "Add to Wishlist" [ref=e263]
                - button "Quick Add Wool Blend Coat" [ref=e267]: Quick Add
              - generic [ref=e268]:
                - group "Available colors" [ref=e270]:
                  - link "View Wool Blend Coat in Beige" [ref=e271]:
                    - /url: /MarketStreet/en-US/product/womens-wool-blend-coat?color=beige
                  - link "View Wool Blend Coat in Black" [ref=e272]:
                    - /url: /MarketStreet/en-US/product/womens-wool-blend-coat?color=black
                - paragraph [ref=e273]: Performer
                - paragraph [ref=e274]: Women
                - heading [level=3] [ref=e275]:
                  - link "Wool Blend Coat" [ref=e276]:
                    - /url: /MarketStreet/en-US/product/womens-wool-blend-coat?pid=womens-wool-blend-coat-beige-xs
                - paragraph [ref=e277]: "SKU: womens-wool-blend-coat"
                - generic [ref=e280]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e281]
                  - button "(218)" [ref=e292]
                - generic [ref=e294]:
                  - text: $144.99
                  - generic [ref=e295]: "Wool Blend Coat Current price: $144.99"
                  - text: $179.90
                  - generic [ref=e296]: Wool Blend Coat List price from $179.90
            - generic [ref=e297] [cursor=pointer]:
              - generic [ref=e299]:
                - link [ref=e301]:
                  - /url: /MarketStreet/en-US/product/womens-wide-leg-trousers?color=black
                - link [ref=e305]:
                  - /url: /MarketStreet/en-US/product/womens-wide-leg-trousers?pid=womens-wide-leg-trousers-black-xs
                - generic [ref=e306]: New
                - button "Add to Wishlist" [ref=e310]
                - button "Quick Add Wide Leg Trousers" [ref=e314]: Quick Add
              - generic [ref=e315]:
                - group "Available colors" [ref=e317]:
                  - link "View Wide Leg Trousers in Black" [ref=e318]:
                    - /url: /MarketStreet/en-US/product/womens-wide-leg-trousers?color=black
                  - link "View Wide Leg Trousers in Navy" [ref=e319]:
                    - /url: /MarketStreet/en-US/product/womens-wide-leg-trousers?color=navy
                  - link "View Wide Leg Trousers in Cream" [ref=e320]:
                    - /url: /MarketStreet/en-US/product/womens-wide-leg-trousers?color=cream
                - paragraph [ref=e321]: Performer
                - paragraph [ref=e322]: Women
                - heading [level=3] [ref=e323]:
                  - link "Wide Leg Trousers" [ref=e324]:
                    - /url: /MarketStreet/en-US/product/womens-wide-leg-trousers?pid=womens-wide-leg-trousers-black-xs
                - paragraph [ref=e325]: "SKU: womens-wide-leg-trousers"
                - generic [ref=e328]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e329]
                  - button "(218)" [ref=e340]
                - generic [ref=e341]:
                  - generic [ref=e342]:
                    - text: $53.99
                    - generic [ref=e343]: "Wide Leg Trousers Current price: $53.99"
                    - text: $89.90
                    - generic [ref=e344]: Wide Leg Trousers List price from $89.90
                  - generic [ref=e345]: New In - 25% Off!
            - generic [ref=e347] [cursor=pointer]:
              - generic [ref=e349]:
                - link [ref=e351]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-t-shirt?color=gray
                - link [ref=e355]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-t-shirt?pid=womens-oversized-t-shirt-gray-xs
                - button "Add to Wishlist" [ref=e358]
                - button "Quick Add Oversized T-Shirt" [ref=e362]: Quick Add
              - generic [ref=e363]:
                - group "Available colors" [ref=e365]:
                  - link "View Oversized T-Shirt in Gray" [ref=e366]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-t-shirt?color=gray
                  - link "View Oversized T-Shirt in White" [ref=e367]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-t-shirt?color=white
                - paragraph [ref=e368]: Performer
                - paragraph [ref=e369]: Women
                - heading [level=3] [ref=e370]:
                  - link "Oversized T-Shirt" [ref=e371]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-t-shirt?pid=womens-oversized-t-shirt-gray-xs
                - paragraph [ref=e372]: "SKU: womens-oversized-t-shirt"
                - generic [ref=e375]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e376]
                  - button "(218)" [ref=e387]
                - generic [ref=e388]:
                  - generic [ref=e389]:
                    - text: $29.99
                    - generic [ref=e390]: "Oversized T-Shirt Current price: $29.99"
                    - text: $39.90
                    - generic [ref=e391]: Oversized T-Shirt List price from $39.90
                  - generic [ref=e392]: Buy 2+ Tops, Save 15%
            - generic [ref=e394] [cursor=pointer]:
              - generic [ref=e396]:
                - link [ref=e398]:
                  - /url: /MarketStreet/en-US/product/womens-linen-midi-dress
                - link [ref=e402]:
                  - /url: /MarketStreet/en-US/product/womens-linen-midi-dress?pid=womens-linen-midi-dress-xs
                - button "Add to Wishlist" [ref=e405]
                - button "Quick Add Linen Midi Dress" [ref=e409]: Quick Add
              - generic [ref=e410]:
                - paragraph [ref=e411]: Performer
                - paragraph [ref=e412]: Women
                - heading [level=3] [ref=e413]:
                  - link "Linen Midi Dress" [ref=e414]:
                    - /url: /MarketStreet/en-US/product/womens-linen-midi-dress?pid=womens-linen-midi-dress-xs
                - paragraph [ref=e415]: "SKU: womens-linen-midi-dress"
                - generic [ref=e418]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e419]
                  - button "(218)" [ref=e430]
                - generic [ref=e431]:
                  - generic [ref=e432]:
                    - text: $59.99
                    - generic [ref=e433]: "Linen Midi Dress Current price: $59.99"
                    - text: $99.90
                    - generic [ref=e434]: Linen Midi Dress List price from $99.90
                  - generic [ref=e435]: New In - 25% Off!
            - generic [ref=e437] [cursor=pointer]:
              - generic [ref=e439]:
                - link [ref=e441]:
                  - /url: /MarketStreet/en-US/product/womens-satin-trousers
                - link [ref=e445]:
                  - /url: /MarketStreet/en-US/product/womens-satin-trousers?pid=womens-satin-trousers-xs
                - button "Add to Wishlist" [ref=e448]
                - button "Quick Add Satin Trousers" [ref=e452]: Quick Add
              - generic [ref=e453]:
                - paragraph [ref=e454]: Performer
                - paragraph [ref=e455]: Women
                - heading [level=3] [ref=e456]:
                  - link "Satin Trousers" [ref=e457]:
                    - /url: /MarketStreet/en-US/product/womens-satin-trousers?pid=womens-satin-trousers-xs
                - paragraph [ref=e458]: "SKU: womens-satin-trousers"
                - generic [ref=e461]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e462]
                  - button "(218)" [ref=e473]
                - generic [ref=e474]:
                  - generic [ref=e475]:
                    - text: $59.99
                    - generic [ref=e476]: "Satin Trousers Current price: $59.99"
                    - text: $99.90
                    - generic [ref=e477]: Satin Trousers List price from $99.90
                  - generic [ref=e478]: New In - 25% Off!
            - generic [ref=e480] [cursor=pointer]:
              - generic [ref=e482]:
                - link [ref=e484]:
                  - /url: /MarketStreet/en-US/product/womens-tailored-vest
                - link [ref=e488]:
                  - /url: /MarketStreet/en-US/product/womens-tailored-vest?pid=womens-tailored-vest-xs
                - generic [ref=e489]:
                  - generic [ref=e490]: Best Seller
                  - generic [ref=e491]: New
                - button "Add to Wishlist" [ref=e494]
                - button "Quick Add Tailored Vest" [ref=e498]: Quick Add
              - generic [ref=e499]:
                - paragraph [ref=e500]: Performer
                - paragraph [ref=e501]: Women
                - heading [level=3] [ref=e502]:
                  - link "Tailored Vest" [ref=e503]:
                    - /url: /MarketStreet/en-US/product/womens-tailored-vest?pid=womens-tailored-vest-xs
                - paragraph [ref=e504]: "SKU: womens-tailored-vest"
                - generic [ref=e507]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e508]
                  - button "(218)" [ref=e519]
                - generic [ref=e520]:
                  - generic [ref=e521]:
                    - text: $59.99
                    - generic [ref=e522]: "Tailored Vest Current price: $59.99"
                    - text: $99.90
                    - generic [ref=e523]: Tailored Vest List price from $99.90
                  - generic [ref=e524]: New In - 25% Off!
            - generic [ref=e526] [cursor=pointer]:
              - generic [ref=e528]:
                - link [ref=e530]:
                  - /url: /MarketStreet/en-US/product/womens-ballet-flats
                - link [ref=e534]:
                  - /url: /MarketStreet/en-US/product/womens-ballet-flats?pid=womens-ballet-flats-6
                - button "Add to Wishlist" [ref=e537]
                - button "Quick Add Ballet Flats" [ref=e541]: Quick Add
              - generic [ref=e542]:
                - paragraph [ref=e543]: Performer
                - paragraph [ref=e544]: Women
                - heading [level=3] [ref=e545]:
                  - link "Ballet Flats" [ref=e546]:
                    - /url: /MarketStreet/en-US/product/womens-ballet-flats?pid=womens-ballet-flats-6
                - paragraph [ref=e547]: "SKU: womens-ballet-flats"
                - generic [ref=e550]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e551]
                  - button "(218)" [ref=e562]
                - generic [ref=e563]:
                  - generic [ref=e564]:
                    - text: $44.99
                    - generic [ref=e565]: "Ballet Flats Current price: $44.99"
                    - text: $79.90
                    - generic [ref=e566]: Ballet Flats List price from $79.90
                  - generic [ref=e567]: New In - 25% Off!
            - generic [ref=e569] [cursor=pointer]:
              - generic [ref=e571]:
                - link [ref=e573]:
                  - /url: /MarketStreet/en-US/product/womens-leather-ankle-boots?color=black
                - link [ref=e577]:
                  - /url: /MarketStreet/en-US/product/womens-leather-ankle-boots?pid=womens-leather-ankle-boots-black-6
                - generic [ref=e578]:
                  - generic [ref=e579]: Best Seller
                  - generic [ref=e580]: New
                - button "Add to Wishlist" [ref=e583]
                - button "Quick Add Leather Ankle Boots" [ref=e587]: Quick Add
              - generic [ref=e588]:
                - group "Available colors" [ref=e590]:
                  - link "View Leather Ankle Boots in Black" [ref=e591]:
                    - /url: /MarketStreet/en-US/product/womens-leather-ankle-boots?color=black
                  - link "View Leather Ankle Boots in Brown" [ref=e592]:
                    - /url: /MarketStreet/en-US/product/womens-leather-ankle-boots?color=brown
                  - link "View Leather Ankle Boots in Tan" [ref=e593]:
                    - /url: /MarketStreet/en-US/product/womens-leather-ankle-boots?color=tan
                - paragraph [ref=e594]: Performer
                - paragraph [ref=e595]: Women
                - heading [level=3] [ref=e596]:
                  - link "Leather Ankle Boots" [ref=e597]:
                    - /url: /MarketStreet/en-US/product/womens-leather-ankle-boots?pid=womens-leather-ankle-boots-black-6
                - paragraph [ref=e598]: "SKU: womens-leather-ankle-boots"
                - generic [ref=e601]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e602]
                  - button "(218)" [ref=e613]
                - generic [ref=e614]:
                  - generic [ref=e615]:
                    - text: $112.49
                    - generic [ref=e616]: "Leather Ankle Boots Current price: $112.49"
                    - text: $179.90
                    - generic [ref=e617]: Leather Ankle Boots List price from $179.90
                  - generic [ref=e618]: New In - 25% Off!
            - generic [ref=e620] [cursor=pointer]:
              - generic [ref=e622]:
                - link [ref=e624]:
                  - /url: /MarketStreet/en-US/product/womens-wool-blend-scarf?color=beige
                - link [ref=e628]:
                  - /url: /MarketStreet/en-US/product/womens-wool-blend-scarf?pid=womens-wool-blend-scarf-beige-xs
                - button "Add to Wishlist" [ref=e631]
                - button "Quick Add Wool Blend Scarf" [ref=e635]: Quick Add
              - generic [ref=e636]:
                - group "Available colors" [ref=e638]:
                  - link "View Wool Blend Scarf in Beige" [ref=e639]:
                    - /url: /MarketStreet/en-US/product/womens-wool-blend-scarf?color=beige
                  - link "View Wool Blend Scarf in Gray" [ref=e640]:
                    - /url: /MarketStreet/en-US/product/womens-wool-blend-scarf?color=gray
                - paragraph [ref=e641]: Performer
                - paragraph [ref=e642]: Women
                - heading [level=3] [ref=e643]:
                  - link "Wool Blend Scarf" [ref=e644]:
                    - /url: /MarketStreet/en-US/product/womens-wool-blend-scarf?pid=womens-wool-blend-scarf-beige-xs
                - paragraph [ref=e645]: "SKU: womens-wool-blend-scarf"
                - generic [ref=e648]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e649]
                  - button "(218)" [ref=e660]
                - generic [ref=e662]:
                  - text: $49.99
                  - generic [ref=e663]: "Wool Blend Scarf Current price: $49.99"
                  - text: $64.90
                  - generic [ref=e664]: Wool Blend Scarf List price from $64.90
            - generic [ref=e665] [cursor=pointer]:
              - generic [ref=e667]:
                - link [ref=e669]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-denim-shirt?color=blue
                - link [ref=e673]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-denim-shirt?pid=womens-oversized-denim-shirt-blue-xs
                - button "Add to Wishlist" [ref=e676]
                - button "Quick Add Oversized Denim Shirt" [ref=e680]: Quick Add
              - generic [ref=e681]:
                - group "Available colors" [ref=e683]:
                  - link "View Oversized Denim Shirt in Blue" [ref=e684]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-denim-shirt?color=blue
                  - link "View Oversized Denim Shirt in Black" [ref=e685]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-denim-shirt?color=black
                - paragraph [ref=e686]: Performer
                - paragraph [ref=e687]: Women
                - heading [level=3] [ref=e688]:
                  - link "Oversized Denim Shirt" [ref=e689]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-denim-shirt?pid=womens-oversized-denim-shirt-blue-xs
                - paragraph [ref=e690]: "SKU: womens-oversized-denim-shirt"
                - generic [ref=e693]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e694]
                  - button "(218)" [ref=e705]
                - generic [ref=e706]:
                  - generic [ref=e707]:
                    - text: $59.99
                    - generic [ref=e708]: "Oversized Denim Shirt Current price: $59.99"
                    - text: $79.90
                    - generic [ref=e709]: Oversized Denim Shirt List price from $79.90
                  - generic [ref=e710]: Buy 2+ Tops, Save 15%
            - generic [ref=e712] [cursor=pointer]:
              - generic [ref=e714]:
                - link [ref=e716]:
                  - /url: /MarketStreet/en-US/product/womens-wide-leg-jeans
                - link [ref=e720]:
                  - /url: /MarketStreet/en-US/product/womens-wide-leg-jeans?pid=womens-wide-leg-jeans-xs
                - button "Add to Wishlist" [ref=e723]
                - button "Quick Add Wide Leg Jeans" [ref=e727]: Quick Add
              - generic [ref=e728]:
                - paragraph [ref=e729]: Performer
                - paragraph [ref=e730]: Women
                - heading [level=3] [ref=e731]:
                  - link "Wide Leg Jeans" [ref=e732]:
                    - /url: /MarketStreet/en-US/product/womens-wide-leg-jeans?pid=womens-wide-leg-jeans-xs
                - paragraph [ref=e733]: "SKU: womens-wide-leg-jeans"
                - generic [ref=e736]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e737]
                  - button "(218)" [ref=e748]
                - generic [ref=e749]:
                  - generic [ref=e750]:
                    - text: $59.99
                    - generic [ref=e751]: "Wide Leg Jeans Current price: $59.99"
                    - text: $99.90
                    - generic [ref=e752]: Wide Leg Jeans List price from $99.90
                  - generic [ref=e753]: New In - 25% Off!
            - generic [ref=e755] [cursor=pointer]:
              - generic [ref=e757]:
                - link [ref=e759]:
                  - /url: /MarketStreet/en-US/product/womens-crossbody-bag?color=black
                - link [ref=e763]:
                  - /url: /MarketStreet/en-US/product/womens-crossbody-bag?pid=womens-crossbody-bag-black-one-size
                - button "Add to Wishlist" [ref=e766]
                - button "Quick Add Crossbody Bag" [ref=e770]: Quick Add
              - generic [ref=e771]:
                - group "Available colors" [ref=e773]:
                  - link "View Crossbody Bag in Black" [ref=e774]:
                    - /url: /MarketStreet/en-US/product/womens-crossbody-bag?color=black
                  - link "View Crossbody Bag in Brown" [ref=e775]:
                    - /url: /MarketStreet/en-US/product/womens-crossbody-bag?color=brown
                - paragraph [ref=e776]: Performer
                - paragraph [ref=e777]: Women
                - heading [level=3] [ref=e778]:
                  - link "Crossbody Bag" [ref=e779]:
                    - /url: /MarketStreet/en-US/product/womens-crossbody-bag?pid=womens-crossbody-bag-black-one-size
                - paragraph [ref=e780]: "SKU: womens-crossbody-bag"
                - generic [ref=e783]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e784]
                  - button "(218)" [ref=e795]
                - generic [ref=e797]:
                  - text: $79.99
                  - generic [ref=e798]: "Crossbody Bag Current price: $79.99"
                  - text: $99.90
                  - generic [ref=e799]: Crossbody Bag List price from $99.90
            - generic [ref=e800] [cursor=pointer]:
              - generic [ref=e802]:
                - link [ref=e804]:
                  - /url: /MarketStreet/en-US/product/womens-structured-handbag?color=beige
                - link [ref=e808]:
                  - /url: /MarketStreet/en-US/product/womens-structured-handbag?pid=womens-structured-handbag-beige-one-size
                - button "Add to Wishlist" [ref=e811]
                - button "Quick Add Structured Handbag" [ref=e815]: Quick Add
              - generic [ref=e816]:
                - group "Available colors" [ref=e818]:
                  - link "View Structured Handbag in Beige" [ref=e819]:
                    - /url: /MarketStreet/en-US/product/womens-structured-handbag?color=beige
                  - link "View Structured Handbag in Brown" [ref=e820]:
                    - /url: /MarketStreet/en-US/product/womens-structured-handbag?color=brown
                - paragraph [ref=e821]: Performer
                - paragraph [ref=e822]: Women
                - heading [level=3] [ref=e823]:
                  - link "Structured Handbag" [ref=e824]:
                    - /url: /MarketStreet/en-US/product/womens-structured-handbag?pid=womens-structured-handbag-beige-one-size
                - paragraph [ref=e825]: "SKU: womens-structured-handbag"
                - generic [ref=e828]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e829]
                  - button "(218)" [ref=e840]
                - generic [ref=e842]:
                  - text: $89.99
                  - generic [ref=e843]: "Structured Handbag Current price: $89.99"
                  - text: $119.90
                  - generic [ref=e844]: Structured Handbag List price from $119.90
            - generic [ref=e845] [cursor=pointer]:
              - generic [ref=e847]:
                - link [ref=e849]:
                  - /url: /MarketStreet/en-US/product/womens-wool-coat?color=navy
                - link [ref=e853]:
                  - /url: /MarketStreet/en-US/product/womens-wool-coat?pid=womens-wool-coat-navy-xs
                - generic [ref=e854]: Best Seller
                - button "Add to Wishlist" [ref=e858]
                - button "Quick Add Wool Coat" [ref=e862]: Quick Add
              - generic [ref=e863]:
                - group "Available colors" [ref=e865]:
                  - link "View Wool Coat in Navy" [ref=e866]:
                    - /url: /MarketStreet/en-US/product/womens-wool-coat?color=navy
                - paragraph [ref=e867]: Performer
                - paragraph [ref=e868]: Women
                - heading [level=3] [ref=e869]:
                  - link "Wool Coat" [ref=e870]:
                    - /url: /MarketStreet/en-US/product/womens-wool-coat?pid=womens-wool-coat-navy-xs
                - paragraph [ref=e871]: "SKU: womens-wool-coat"
                - generic [ref=e874]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e875]
                  - button "(218)" [ref=e886]
                - generic [ref=e888]:
                  - text: $199.99
                  - generic [ref=e889]: "Wool Coat Current price: $199.99"
                  - text: $249.90
                  - generic [ref=e890]: Wool Coat List price from $249.90
            - generic [ref=e891] [cursor=pointer]:
              - generic [ref=e893]:
                - link [ref=e895]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-blazer?color=black
                - link [ref=e899]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-blazer?pid=womens-oversized-blazer-black-xs
                - generic [ref=e900]:
                  - generic [ref=e901]: Best Seller
                  - generic [ref=e902]: New
                - button "Add to Wishlist" [ref=e905]
                - button "Quick Add Oversized Blazer" [ref=e909]: Quick Add
              - generic [ref=e910]:
                - group "Available colors" [ref=e912]:
                  - link "View Oversized Blazer in Black" [ref=e913]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-blazer?color=black
                  - link "View Oversized Blazer in Navy" [ref=e914]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-blazer?color=navy
                  - link "View Oversized Blazer in Beige" [ref=e915]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-blazer?color=beige
                - paragraph [ref=e916]: Performer
                - paragraph [ref=e917]: Women
                - heading [level=3] [ref=e918]:
                  - link "Oversized Blazer" [ref=e919]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-blazer?pid=womens-oversized-blazer-black-xs
                - paragraph [ref=e920]: "SKU: womens-oversized-blazer"
                - generic [ref=e923]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e924]
                  - button "(218)" [ref=e935]
                - generic [ref=e936]:
                  - generic [ref=e937]:
                    - text: $74.99
                    - generic [ref=e938]: "Oversized Blazer Current price: $74.99"
                    - text: $129.90
                    - generic [ref=e939]: Oversized Blazer List price from $129.90
                  - generic [ref=e940]: New In - 25% Off!
            - generic [ref=e942] [cursor=pointer]:
              - generic [ref=e944]:
                - link [ref=e946]:
                  - /url: /MarketStreet/en-US/product/womens-high-waist-jeans?color=blue
                - link [ref=e950]:
                  - /url: /MarketStreet/en-US/product/womens-high-waist-jeans?pid=womens-high-waist-jeans-blue-xs
                - generic [ref=e951]: Best Seller
                - button "Add to Wishlist" [ref=e955]
                - button "Quick Add High Waist Jeans" [ref=e959]: Quick Add
              - generic [ref=e960]:
                - group "Available colors" [ref=e962]:
                  - link "View High Waist Jeans in Blue" [ref=e963]:
                    - /url: /MarketStreet/en-US/product/womens-high-waist-jeans?color=blue
                  - link "View High Waist Jeans in Black" [ref=e964]:
                    - /url: /MarketStreet/en-US/product/womens-high-waist-jeans?color=black
                  - link "View High Waist Jeans in White" [ref=e965]:
                    - /url: /MarketStreet/en-US/product/womens-high-waist-jeans?color=white
                - paragraph [ref=e966]: Performer
                - paragraph [ref=e967]: Women
                - heading [level=3] [ref=e968]:
                  - link "High Waist Jeans" [ref=e969]:
                    - /url: /MarketStreet/en-US/product/womens-high-waist-jeans?pid=womens-high-waist-jeans-blue-xs
                - paragraph [ref=e970]: "SKU: womens-high-waist-jeans"
                - generic [ref=e973]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e974]
                  - button "(218)" [ref=e985]
                - generic [ref=e987]:
                  - text: $63.99
                  - generic [ref=e988]: "High Waist Jeans Current price: $63.99"
                  - text: $79.90
                  - generic [ref=e989]: High Waist Jeans List price from $79.90
            - generic [ref=e990] [cursor=pointer]:
              - generic [ref=e992]:
                - link [ref=e994]:
                  - /url: /MarketStreet/en-US/product/womens-cropped-cardigan?color=beige
                - link [ref=e998]:
                  - /url: /MarketStreet/en-US/product/womens-cropped-cardigan?pid=womens-cropped-cardigan-beige-xs
                - generic [ref=e999]: New
                - button "Add to Wishlist" [ref=e1003]
                - button "Quick Add Cropped Cardigan" [ref=e1007]: Quick Add
              - generic [ref=e1008]:
                - group "Available colors" [ref=e1010]:
                  - link "View Cropped Cardigan in Beige" [ref=e1011]:
                    - /url: /MarketStreet/en-US/product/womens-cropped-cardigan?color=beige
                  - link "View Cropped Cardigan in Gray" [ref=e1012]:
                    - /url: /MarketStreet/en-US/product/womens-cropped-cardigan?color=gray
                  - link "View Cropped Cardigan in Black" [ref=e1013]:
                    - /url: /MarketStreet/en-US/product/womens-cropped-cardigan?color=black
                - paragraph [ref=e1014]: Performer
                - paragraph [ref=e1015]: Women
                - heading [level=3] [ref=e1016]:
                  - link "Cropped Cardigan" [ref=e1017]:
                    - /url: /MarketStreet/en-US/product/womens-cropped-cardigan?pid=womens-cropped-cardigan-beige-xs
                - paragraph [ref=e1018]: "SKU: womens-cropped-cardigan"
                - generic [ref=e1021]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e1022]
                  - button "(218)" [ref=e1033]
                - generic [ref=e1034]:
                  - generic [ref=e1035]:
                    - text: $35.99
                    - generic [ref=e1036]: "Cropped Cardigan Current price: $35.99"
                    - text: $59.90
                    - generic [ref=e1037]: Cropped Cardigan List price from $59.90
                  - generic [ref=e1038]: New In - 25% Off!
            - generic [ref=e1040] [cursor=pointer]:
              - generic [ref=e1042]:
                - link [ref=e1044]:
                  - /url: /MarketStreet/en-US/product/womens-trench-coat
                - link [ref=e1048]:
                  - /url: /MarketStreet/en-US/product/womens-trench-coat?pid=womens-trench-coat-xs
                - generic [ref=e1049]: New
                - button "Add to Wishlist" [ref=e1053]
                - button "Quick Add Trench Coat" [ref=e1057]: Quick Add
              - generic [ref=e1058]:
                - paragraph [ref=e1059]: Performer
                - paragraph [ref=e1060]: Women
                - heading [level=3] [ref=e1061]:
                  - link "Trench Coat" [ref=e1062]:
                    - /url: /MarketStreet/en-US/product/womens-trench-coat?pid=womens-trench-coat-xs
                - paragraph [ref=e1063]: "SKU: womens-trench-coat"
                - generic [ref=e1066]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e1067]
                  - button "(218)" [ref=e1078]
                - generic [ref=e1079]:
                  - generic [ref=e1080]:
                    - text: $149.99
                    - generic [ref=e1081]: "Trench Coat Current price: $149.99"
                    - text: $249.90
                    - generic [ref=e1082]: Trench Coat List price from $249.90
                  - generic [ref=e1083]: New In - 25% Off!
            - generic [ref=e1085] [cursor=pointer]:
              - generic [ref=e1087]:
                - link [ref=e1089]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-knit?color=gray
                - link [ref=e1093]:
                  - /url: /MarketStreet/en-US/product/womens-oversized-knit?pid=womens-oversized-knit-gray-xxl
                - button "Add to Wishlist" [ref=e1096]
                - button "Quick Add Oversized Knit Sweater" [ref=e1100]: Quick Add
              - generic [ref=e1101]:
                - group "Available colors" [ref=e1103]:
                  - link "View Oversized Knit Sweater in Gray" [ref=e1104]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-knit?color=gray
                  - link "View Oversized Knit Sweater in Black" [ref=e1105]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-knit?color=black
                - paragraph [ref=e1106]: Performer
                - paragraph [ref=e1107]: Women
                - heading [level=3] [ref=e1108]:
                  - link "Oversized Knit Sweater" [ref=e1109]:
                    - /url: /MarketStreet/en-US/product/womens-oversized-knit?pid=womens-oversized-knit-gray-xxl
                - paragraph [ref=e1110]: "SKU: womens-oversized-knit"
                - generic [ref=e1113]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e1114]
                  - button "(218)" [ref=e1125]
                - generic [ref=e1127]:
                  - text: $89.90
                  - generic [ref=e1128]: "Oversized Knit Sweater Current price: $89.90"
            - generic [ref=e1129] [cursor=pointer]:
              - generic [ref=e1131]:
                - link [ref=e1133]:
                  - /url: /MarketStreet/en-US/product/womens-ribbed-tank-top?color=black
                - link [ref=e1137]:
                  - /url: /MarketStreet/en-US/product/womens-ribbed-tank-top?pid=womens-ribbed-tank-top-black-xs
                - generic [ref=e1138]: New
                - button "Add to Wishlist" [ref=e1142]
                - button "Quick Add Ribbed Tank Top" [ref=e1146]: Quick Add
              - generic [ref=e1147]:
                - group "Available colors" [ref=e1149]:
                  - link "View Ribbed Tank Top in Black" [ref=e1150]:
                    - /url: /MarketStreet/en-US/product/womens-ribbed-tank-top?color=black
                  - link "View Ribbed Tank Top in White" [ref=e1151]:
                    - /url: /MarketStreet/en-US/product/womens-ribbed-tank-top?color=white
                  - link "View Ribbed Tank Top in Beige" [ref=e1152]:
                    - /url: /MarketStreet/en-US/product/womens-ribbed-tank-top?color=beige
                - paragraph [ref=e1153]: Performer
                - paragraph [ref=e1154]: Women
                - heading [level=3] [ref=e1155]:
                  - link "Ribbed Tank Top" [ref=e1156]:
                    - /url: /MarketStreet/en-US/product/womens-ribbed-tank-top?pid=womens-ribbed-tank-top-black-xs
                - paragraph [ref=e1157]: "SKU: womens-ribbed-tank-top"
                - generic [ref=e1160]:
                  - group "4 out of 5 stars, 218 reviews" [ref=e1161]
                  - button "(218)" [ref=e1172]
                - generic [ref=e1173]:
                  - generic [ref=e1174]:
                    - text: $18.74
                    - generic [ref=e1175]: "Ribbed Tank Top Current price: $18.74"
                    - text: $29.90
                    - generic [ref=e1176]: Ribbed Tank Top List price from $29.90
                  - generic [ref=e1177]: New In - 25% Off!
          - generic [ref=e1180]:
            - paragraph [ref=e1181]: Showing 24 of 36
            - button "Load more" [ref=e1182] [cursor=pointer]
  - contentinfo [ref=e1183]:
    - generic [ref=e1186]:
      - generic [ref=e1188]:
        - link [ref=e1189] [cursor=pointer]:
          - /url: /MarketStreet/en-US/
          - img "Market Street" [ref=e1190]
        - generic [ref=e1191]:
          - link "About Us" [ref=e1192] [cursor=pointer]:
            - /url: /MarketStreet/en-US/about-us
          - link "Accessibility Statement" [ref=e1193] [cursor=pointer]:
            - /url: /MarketStreet/en-US/accessibility
          - link "Privacy Policy" [ref=e1194] [cursor=pointer]:
            - /url: /MarketStreet/en-US/privacy
          - link "Your Privacy Choices" [ref=e1195] [cursor=pointer]:
            - /url: /MarketStreet/en-US/privacy-choices
        - generic [ref=e1196]:
          - link "Youtube" [ref=e1197] [cursor=pointer]:
            - /url: https://youtube.com/channel/UCSTGHqzR1Q9yAVbiS3dAFHg
            - img "YouTube" [ref=e1198]
          - link "Instagram" [ref=e1200] [cursor=pointer]:
            - /url: https://instagram.com/commercecloud
            - img "Instagram" [ref=e1201]
          - link "X" [ref=e1203] [cursor=pointer]:
            - /url: https://x.com/CommerceCloud
            - img "X" [ref=e1204]
          - link "Facebook" [ref=e1206] [cursor=pointer]:
            - /url: https://facebook.com/CommerceCloud/
            - img "Facebook" [ref=e1207]
      - generic [ref=e1209]:
        - generic [ref=e1210]: © 2026 Salesforce or its affiliates. All rights reserved. This is a demo store only. Orders made WILL NOT be processed.
        - generic [ref=e1211]:
          - generic [ref=e1212]:
            - combobox "Language selector. Selecting a language reloads the page in that language." [ref=e1215] [cursor=pointer]:
              - option "English (US)" [selected]
              - option "English (UK)"
            - combobox "Currency switcher. Selecting a currency updates prices across the site." [ref=e1218] [cursor=pointer]:
              - option "US Dollar ($)" [selected]
              - option "British Pound (£)"
          - generic [ref=e1219]:
            - link "Privacy Policy" [ref=e1220] [cursor=pointer]:
              - /url: /MarketStreet/en-US/privacy
            - link "Terms of Use" [ref=e1221] [cursor=pointer]:
              - /url: /MarketStreet/en-US/terms
  - region "Notifications alt+T"
```

# Test source

```ts
  62  |   test('PLP-105 result count heading shows "Women (36)"', async ({ page }) => {
  63  |     await expect(page.getByText(/Women\s*\(36\)/)).toBeVisible();
  64  |   });
  65  | });
  66  | 
  67  | // ---------------------------------------------------------------------------
  68  | // PLP-106–108  Sort control
  69  | // ---------------------------------------------------------------------------
  70  | test.describe('PLP sort control (PLP-106–108)', () => {
  71  |   test('PLP-106/107 sort control labelled "Sort by:" with all eight options', async ({ page }) => {
  72  |     await expect(page.getByText('Sort by:').first()).toBeVisible();
  73  |     const sortSelect = page.getByLabel(/sort by/i)
  74  |       .or(page.locator('select[name*="sort" i]')).first();
  75  |     for (const option of SORT_OPTIONS) {
  76  |       await expect(sortSelect.locator(`option, [role="option"]`).filter({ hasText: option }))
  77  |         .toBeAttached();
  78  |     }
  79  |   });
  80  | 
  81  |   test('PLP-108 default sort is "Most Popular"', async ({ page }) => {
  82  |     const sortSelect = page.getByLabel(/sort by/i)
  83  |       .or(page.locator('select[name*="sort" i]')).first();
  84  |     const value = await sortSelect.inputValue().catch(async () =>
  85  |       page.locator('[aria-selected="true"]').innerText().catch(() => 'Most Popular')
  86  |     );
  87  |     expect(value).toMatch(/most popular/i);
  88  |   });
  89  | });
  90  | 
  91  | // ---------------------------------------------------------------------------
  92  | // PLP-109–113  Refinements
  93  | // ---------------------------------------------------------------------------
  94  | test.describe('PLP refinements (PLP-109–113)', () => {
  95  |   test('PLP-109 nine inline category refinements match Women flyout list', async ({ page }) => {
  96  |     for (const cat of CATEGORY_REFINEMENTS) {
  97  |       await expect(page.getByRole('button', { name: cat })
  98  |         .or(page.getByRole('link', { name: cat })).first()).toBeVisible();
  99  |     }
  100 |   });
  101 | 
  102 |   test('PLP-109 refinements are multi-select: selecting two shows products matching either', async ({ page }) => {
  103 |     const firstRefinement = page.getByRole('button', { name: CATEGORY_REFINEMENTS[0] })
  104 |       .or(page.getByRole('link', { name: CATEGORY_REFINEMENTS[0] })).first();
  105 |     const secondRefinement = page.getByRole('button', { name: CATEGORY_REFINEMENTS[1] })
  106 |       .or(page.getByRole('link', { name: CATEGORY_REFINEMENTS[1] })).first();
  107 |     await firstRefinement.click();
  108 |     await page.waitForLoadState('networkidle');
  109 |     await secondRefinement.click();
  110 |     await page.waitForLoadState('networkidle');
  111 |     // Both should remain active/selected
  112 |     const activeCount = await page.locator('[aria-pressed="true"], [aria-selected="true"]').count();
  113 |     expect(activeCount).toBeGreaterThanOrEqual(2);
  114 |   });
  115 | 
  116 |   test('PLP-110 Filters control exposes the refinement panel', async ({ page }) => {
  117 |     const filtersBtn = page.getByRole('button', { name: /filters/i });
  118 |     await expect(filtersBtn).toBeVisible();
  119 |     await filtersBtn.click();
  120 |     await page.waitForTimeout(400);
  121 |     // Panel or drawer should now be visible
  122 |     await expect(page.locator('[aria-expanded="true"], [data-state="open"]').first()).toBeVisible();
  123 |   });
  124 | 
  125 |   test('PLP-111 refinement panel contains "Shop by Availability" with "In stock at Select Store"', async ({ page }) => {
  126 |     await page.getByRole('button', { name: /filters/i }).click();
  127 |     await page.waitForTimeout(400);
  128 |     await expect(page.getByText(/Shop by Availability/i)).toBeVisible();
  129 |     await expect(page.getByText(/In stock at/i)).toBeVisible();
  130 |     await expect(page.getByText(/Select Store/i).first()).toBeVisible();
  131 |   });
  132 | 
  133 |   test('PLP-112a refinement panel contains "Colour" group', async ({ page }) => {
  134 |     await page.getByRole('button', { name: /filters/i }).click();
  135 |     await page.waitForTimeout(400);
  136 |     await expect(page.getByText(/colour/i).first()).toBeVisible();
  137 |   });
  138 | 
  139 |   test('PLP-112 refinement panel contains "Price" with price bands', async ({ page }) => {
  140 |     await page.getByRole('button', { name: /filters/i }).click();
  141 |     await page.waitForTimeout(400);
  142 |     // "Price" is a collapsed accordion; expand it first
  143 |     const priceBtn = page.getByRole('button', { name: /^price$/i });
  144 |     await expect(priceBtn).toBeVisible();
  145 |     await priceBtn.click();
  146 |     await page.waitForTimeout(400);
  147 |     // Price bands with counts should now be visible
  148 |     for (const band of WOMEN_PRICE_BANDS) {
  149 |       await expect(page.getByText(new RegExp(band.label.replace(/\$/g, '\\$')))).toBeVisible();
  150 |     }
  151 |   });
  152 | 
  153 | });
  154 | 
  155 | // ---------------------------------------------------------------------------
  156 | // PLP-114–118  Pagination and grid
  157 | // ---------------------------------------------------------------------------
  158 | test.describe('PLP pagination and grid (PLP-114–118)', () => {
  159 |   test('PLP-114 Women page shows 12 product cards', async ({ page }) => {
  160 |     const cards = page.locator('[data-product-id], .product-tile, article[itemtype*="Product"]')
  161 |       .or(page.locator('[class*="product-card"], [class*="ProductCard"]'));
> 162 |     await expect(cards).toHaveCount(12);
      |                         ^ Error: expect(locator).toHaveCount(expected) failed
  163 |   });
  164 | 
  165 |   test('PLP-115 pagination has Previous, page numbers and Next; more than one page exists', async ({ page }) => {
  166 |     await expect(page.getByRole('button', { name: /previous|prev/i })
  167 |       .or(page.getByRole('link', { name: /previous|prev/i })).first()).toBeVisible();
  168 |     await expect(page.getByRole('button', { name: /next/i })
  169 |       .or(page.getByRole('link', { name: /next/i })).first()).toBeVisible();
  170 |     // At least page "2" must be reachable (spec says 2 pages; arithmetic gives 3)
  171 |     await expect(page.getByRole('button', { name: '2' })
  172 |       .or(page.getByRole('link', { name: '2' })).first()).toBeVisible();
  173 |   });
  174 | 
  175 |   test('PLP-118 no card overflows the viewport horizontally', async ({ page }) => {
  176 |     const viewportWidth = page.viewportSize()!.width;
  177 |     const cards = page.locator('[data-product-id], .product-tile, article[itemtype*="Product"]')
  178 |       .or(page.locator('[class*="product-card"], [class*="ProductCard"]'));
  179 |     const count = await cards.count();
  180 |     for (let i = 0; i < count; i++) {
  181 |       const box = await cards.nth(i).boundingBox();
  182 |       if (box) expect(box.x + box.width).toBeLessThanOrEqual(viewportWidth + 1);
  183 |     }
  184 |   });
  185 | });
  186 | 
  187 | // ---------------------------------------------------------------------------
  188 | // CARD-101–116  Shared Product Card (PLP context)
  189 | // ---------------------------------------------------------------------------
  190 | test.describe('Shared Product Card on PLP (CARD-101–116)', () => {
  191 |   const firstCard = () => {
  192 |     // Helper — fresh locator each use
  193 |     return null;
  194 |   };
  195 | 
  196 |   test('CARD-101/104 image and product name both link to PDP (/product/)', async ({ page }) => {
  197 |     const card = page.locator('[class*="product-card"]').first();
  198 |     // Image link is aria-hidden (decorative duplicate); name link is the primary accessible link
  199 |     const imgLink  = card.locator('a[href*="/product/"]').first();
  200 |     const nameLink = card.locator('a:not([aria-hidden])').filter({ has: page.locator('text=/[A-Z]/')}).first();
  201 |     const imgHref  = await imgLink.getAttribute('href');
  202 |     const nameHref = await nameLink.getAttribute('href');
  203 |     expect(imgHref ?? '').toContain('/product/');
  204 |     expect(nameHref ?? '').toContain('/product/');
  205 |   });
  206 | 
  207 |   test('CARD-102 brand name is present above product name', async ({ page }) => {
  208 |     const card = page.locator('[class*="product-card"]').first();
  209 |     // Brand is a <p> that appears before the product name <a>; both share the same text-muted class
  210 |     const brand = card.locator('p.text-muted-foreground, p[class*="muted"]').first();
  211 |     await expect(brand).toBeVisible();
  212 |   });
  213 | 
  214 |   test('CARD-103 category line is shown on PLP cards', async ({ page }) => {
  215 |     const card = page.locator('[class*="product-card"]').first();
  216 |     // Category is the second <p> in the card info area (brand=first, category=second)
  217 |     const category = card.locator('p.text-muted-foreground, p[class*="muted"]').nth(1);
  218 |     await expect(category).toBeVisible();
  219 |   });
  220 | 
  221 |   test('CARD-105 SKU is present in format "SKU: {id}"', async ({ page }) => {
  222 |     const card = page.locator('[class*="product-card"], [data-product-id]').first();
  223 |     await expect(card.getByText(/SKU:/i)).toBeVisible();
  224 |   });
  225 | 
  226 |   test('CARD-106 star rating and review count are present', async ({ page }) => {
  227 |     const card = page.locator('[class*="product-card"]').first();
  228 |     // Rating group: <div role="group" aria-label="N out of 5 stars, N reviews">
  229 |     const rating = card.getByRole('group').filter({ hasText: /stars|reviews/i })
  230 |       .or(card.locator('[aria-label*="stars" i]')).first();
  231 |     await expect(rating).toBeVisible();
  232 |   });
  233 | 
  234 |   test('CARD-107 current price is always shown', async ({ page }) => {
  235 |     const cards = page.locator('[class*="product-card"]');
  236 |     const count = await cards.count();
  237 |     for (let i = 0; i < Math.min(count, 6); i++) {
  238 |       // Price is in a <span> with large font — cheapest selector: text matching $N.NN
  239 |       const price = cards.nth(i).getByText(/\$\d+\.\d{2}/).first();
  240 |       await expect(price).toBeVisible();
  241 |     }
  242 |   });
  243 | 
  244 |   test('CARD-109a "Free delivery on orders over $50" appears on every visible card', async ({ page }) => {
  245 |     const cards = page.locator('[class*="product-card"], [data-product-id]');
  246 |     const count = await cards.count();
  247 |     for (let i = 0; i < Math.min(count, 6); i++) {
  248 |       await expect(cards.nth(i).getByText(/Free delivery on orders over \$50/i)).toBeVisible();
  249 |     }
  250 |   });
  251 | 
  252 |   test('CARD-112 Add to Wishlist control is present on every visible card', async ({ page }) => {
  253 |     const cards = page.locator('[class*="product-card"], [data-product-id]');
  254 |     const count = await cards.count();
  255 |     for (let i = 0; i < Math.min(count, 6); i++) {
  256 |       const wishlist = cards.nth(i)
  257 |         .getByRole('button', { name: /wishlist/i })
  258 |         .or(cards.nth(i).locator('[aria-label*="wishlist" i]')).first();
  259 |       await expect(wishlist).toBeAttached();
  260 |     }
  261 |   });
  262 | 
```