...
- Name: desktop/home.spec.ts >> Global header – GLB-101–110 >> GLB-102 primary nav has four items – Women, Men, Kids, New Arrivals – in order
- Location: tests/desktop/home.spec.ts:62:7
...
Locator: getByRole('navigation').first().getByText('Women', { exact: true }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('navigation').first().getByText('Women', { exact: true }).first()
