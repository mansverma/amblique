...
- Name: mobile/home.spec.ts >> Mobile category tiles (M-HOME-301–309) >> M-HOME-306/307 tile composition: image + gradient overlay + heading; whole tile is click target
- Location: tests/mobile/home.spec.ts:402:7
...
Locator: locator('section').filter({ hasText: 'Style for Real Life' }).first().getByRole('link').first().locator('img').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ hasText: 'Style for Real Life' }).first().getByRole('link').first().locator('img').first()
