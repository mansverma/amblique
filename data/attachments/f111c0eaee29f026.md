...
- Name: desktop/plp.spec.ts >> Shared Product Card on PLP (CARD-101–116) >> CARD-106 star rating and review count are present
- Location: tests/desktop/plp.spec.ts:226:7
...
Locator: locator('[class*="product-card"]').first().getByRole('group').filter({ hasText: /stars|reviews/i }).or(locator('[class*="product-card"]').first().locator('[aria-label*="stars" i]')).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[class*="product-card"]').first().getByRole('group').filter({ hasText: /stars|reviews/i }).or(locator('[class*="product-card"]').first().locator('[aria-label*="stars" i]')).first()
