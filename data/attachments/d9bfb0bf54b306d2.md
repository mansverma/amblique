...
- Name: mobile/plp.spec.ts >> Quick Add modal (M-QA-101–106) >> M-QA-104 modal contains a "View Details" link to the PDP
- Location: tests/mobile/plp.spec.ts:231:7
...
Call log:
  - waiting for locator('[class*="product-card"], [class*="ProductCard"], [data-product-id]').first().getByRole('button', { name: /more options/i }).or(locator('[class*="product-card"], [class*="ProductCard"], [data-product-id]').first().locator('[aria-label*="more" i]')).first()
...
```ts
  ...
  198 | 
  199 | // ---------------------------------------------------------------------------
  200 | // M-QA-101–106  Quick Add modal
  201 | // ---------------------------------------------------------------------------
  202 | test.describe('Quick Add modal (M-QA-101–106)', () => {
  203 |   const openQuickAdd = async (page: any) => {
  204 |     const card       = page.locator('[class*="product-card"], [class*="ProductCard"], [data-product-id]').first();
  205 |     const moreOptions = card.getByRole('button', { name: /more options/i })
  206 |       .or(card.locator('[aria-label*="more" i]')).first();
> 207 |     await moreOptions.tap();
      |                       ^ Error: locator.tap: Test timeout of 30000ms exceeded.
  208 |     await page.waitForTimeout(400);
  209 |   };
  210 | 
  211 |   test('M-QA-101 Quick Add dialog has role="dialog" and accessible name "Quick Add"', async ({ page }) => {
  212 |     await openQuickAdd(page);
  213 |     const dialog = page.getByRole('dialog', { name: /quick add/i });
