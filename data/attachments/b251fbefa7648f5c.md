...
- Name: desktop/home.spec.ts >> Home – Style for Real Life tiles (HOME-301–308) >> HOME-307 Style for Real Life carousel has Previous and Next controls
- Location: tests/desktop/home.spec.ts:363:7
...
Locator: locator('section').filter({ hasText: 'Style for Real Life' }).first().getByRole('button', { name: /previous|prev/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ hasText: 'Style for Real Life' }).first().getByRole('button', { name: /previous|prev/i }).first()
