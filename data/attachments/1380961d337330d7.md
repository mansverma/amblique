...
- Name: mobile/plp.spec.ts >> Mobile product card (M-CARD-101–115) >> M-CARD-105 SKU is shown in format "SKU: {id}"
- Location: tests/mobile/plp.spec.ts:219:7
...
Locator: locator('[class*="product-card"], [class*="ProductCard"], [data-product-id]').first().getByText(/SKU:/i)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[class*="product-card"], [class*="ProductCard"], [data-product-id]').first().getByText(/SKU:/i)
