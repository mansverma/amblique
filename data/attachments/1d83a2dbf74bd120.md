...
- Name: desktop/home.spec.ts >> Home – Style for Real Life tiles (HOME-301–308) >> HOME-305 "Shop Now" label exists in tile markup (collapsed pre-hover)
- Location: tests/desktop/home.spec.ts:341:7
...
Locator: locator('section').filter({ hasText: 'Style for Real Life' }).first().getByText('Shop Now').first()
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ hasText: 'Style for Real Life' }).first().getByText('Shop Now').first()
