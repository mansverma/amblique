...
- Name: mobile/home.spec.ts >> Mobile nav menu (M-GLB-121–131) >> M-GLB-123 four top-level items in order: Women, Men, Kids, New Arrivals
- Location: tests/mobile/home.spec.ts:168:7
...
Locator: locator('nav[aria-label*="Mobile navigation" i]').getByText('Women', { exact: true }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('nav[aria-label*="Mobile navigation" i]').getByText('Women', { exact: true }).first()
