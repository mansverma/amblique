...
- Name: desktop/home.spec.ts >> ENV-09 Tracking consent modal >> presents Accept, Decline and Close controls on first visit
- Location: tests/desktop/home.spec.ts:36:7
...
Locator: locator('[role="dialog"]').filter({ hasText: /consent|cookie|tracking/i }).first().getByRole('button', { name: /accept/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[role="dialog"]').filter({ hasText: /consent|cookie|tracking/i }).first().getByRole('button', { name: /accept/i })
