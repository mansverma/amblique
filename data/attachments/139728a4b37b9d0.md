...
- Name: desktop/home.spec.ts >> Home – Hero carousel (HOME-101–111) >> HOME-102 each active slide contains image, heading, paragraph and CTA link
- Location: tests/desktop/home.spec.ts:222:7
...
Locator: locator('[aria-roledescription="slide"][aria-hidden="false"]').or(locator('[role="group"][aria-label*="1"]')).first().locator('img').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[aria-roledescription="slide"][aria-hidden="false"]').or(locator('[role="group"][aria-label*="1"]')).first().locator('img').first()
    2 × locator resolved to <img loading="eager" fetchpriority="high" class="w-full h-full object-cover" alt="Women's Slacks Jackets and Purses" src="/mobify/bundle/1/client/images/hero-01.webp"/>
