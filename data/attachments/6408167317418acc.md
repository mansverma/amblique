...
- Name: desktop/plp.spec.ts >> Shared Product Card on PLP (CARD-101–116) >> CARD-116 colour swatches each have a 3 px ring on hover
...
Call log:
  - waiting for locator('[class*="product-card"], [data-product-id]').filter({ has: locator('[class*="swatch" i]') }).first().locator('[class*="swatch" i]').first()
