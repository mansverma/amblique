...
- Name: mobile/home.spec.ts >> Touch gestures – home page carousels (M-TCH-101/102/103) >> M-TCH-102 Featured Products carousel is swipeable horizontally by touch
- Location: tests/mobile/home.spec.ts:515:7
...
Call log:
  - waiting for locator('.section-container, section').filter({ hasText: 'Featured Products' }).first().locator('a[href*="/product/"]').first() to be visible
...
> 519 |     await section.locator(`a[href*="/product/"]`).first().waitFor({ state: 'visible' });
  ...
  603 |       await page.waitForTimeout(600);
  604 |       const afterBtnX = await page.evaluate(() => {
  605 |         const s = [...document.querySelectorAll('section')]
  606 |           .find(el => el.textContent?.includes('Style for Real Life'));
  607 |         return s?.querySelector('a[href*="/category/women"]')?.getBoundingClientRect().x ?? null;
  608 |       });
  609 |       expect(afterBtnX, 'Carousel must advance (swipe gesture not testable on WebKit)').not.toBeNull();
  610 |       expect(afterBtnX!).toBeLessThan(coords!.tileX);
  611 |     }
  612 |   });
  613 | });
  614 | 
